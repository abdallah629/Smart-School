<form method="post" action="{{ route('students.promote', [$fc, $fs, $tc, $ts]) }}"> 
            ...
            <tr>
                ...
                <td>
                    <select class="form-control select" name="p-{{$sr->id}}" id="p-{{$sr->id}}">
                        <option value="P">{{ __('ui.promote') }}</option>
                        <option value="D">{{ __("ui.dont_promote") }}</option>
                        <option value="G">{{ __('ui.graduated') }}</option>
                    </select>
                </td>
            </tr>
        @endforeach
    ...
    <div class="text-center mt-3">
        <button class="btn btn-success"><i class="icon-stairs-up mr-2"></i> {{ __('ui.promote_students') }}</button>
    </div>
</form>