...
@section('page_title', __('ui.edit_student'))
@section('content')

<div class="card">
    <div class="card-header bg-white header-elements-inline">
        <h6 id="ajax-title" class="card-title">
            {{ __('ui.fill_form_to_edit', ['name' => $sr->user->name]) }}
        </h6>
        {!! Qs::getPanelOptions() !!}
    </div>
    ...
    <form method="post" enctype="multipart/form-data" class="wizard-form steps-validation ajax-update" data-reload="#ajax-title" action="{{ route('students.update', Qs::hash($sr->id)) }}" data-fouc>
        ...
        <fieldset>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input value="{{ $sr->user->name }}" required type="text" name="name" placeholder="{{ __('ui.full_name') }}" class="form-control">
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input value="{{ $sr->user->address }}" class="form-control" placeholder="{{ __('ui.address') }}" name="address" type="text" required>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        <label>{{ __('ui.email_address') }}: <span class="text-danger">*</span></label>
                        <input value="{{ $sr->user->email }}" type="email" name="email" class="form-control" placeholder="{{ __('ui.email_address') }}">
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select class="select form-control" id="gender" name="gender" required data-fouc data-placeholder="{{ __('ui.choose') }}">
                            <option value=""></option>
                            <option {{ ($sr->user->gender == 'Male' ? 'selected' : '') }} value="Male">{{ __('ui.male') }}</option>
                            <option {{ ($sr->user->gender == 'Female' ? 'selected' : '') }} value="Female">{{ __('ui.female') }}</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input name="dob" value="{{ $sr->user->dob }}" type="text" class="form-control date-pick" placeholder="{{ __('ui.select_date') }}">
                    </div>
                </div>
        ...
        <fieldset>
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="my_class_id">{{ __('ui.class') }}: </label>
                        <select onchange="getClassSections(this.value)" name="my_class_id" required id="my_class_id" class="form-control select-search" data-placeholder="{{ __('ui.select_class') }}">
                            <option value=""></option>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="section_id">{{ __('ui.section') }}: </label>
                        <select name="section_id" required id="section_id" class="form-control select" data-placeholder="{{ __('ui.select_section') }}">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="year_admitted">{{ __('ui.year_admitted') }}: </label>
                        <select name="year_admitted" data-placeholder="{{ __('ui.choose') }}" id="year_admitted" class="select-search form-control">
                            <option value=""></option>
            ...
            <div class="row">
                <div class="col-md-6">
                    <label for="dorm_id">{{ __('ui.dormitory') }}: </label>
                    <select data-placeholder="{{ __('ui.choose') }}" name="dorm_id" id="dorm_id" class="select-search form-control">
                        <option value=""></option>
                        @foreach($dorms as $d)
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label>{{ __('ui.dormitory_room_no') }}:</label>
                        <input type="text" name="dorm_room_no" placeholder="{{ __('ui.dormitory_room_no') }}" class="form-control" value="{{ $sr->dorm_room_no }}">
                    </div>
                </div>
            </div>
        </fieldset>
