<html> 
<head>
    <title>{{ __('ui.receipt') }}_{{ $pr->ref_no.'_'.$sr->user->name }}</title>
    ...
    <div id="print" xmlns:margin-top="http://www.w3.org/1999/xhtml">
        ...
        <table width="100%">
            <tr>
                <td>
                    ...
                    <br/> <br/>
                     <span style="color: #000; font-weight: bold; font-size: 25px;">{{ __('ui.payment_receipt') }}</span>
                </td>
            </tr>
        </table>
        ...
        <div class="bold arial" style="text-align: center; float:right; width: 200px; padding: 5px; margin-right:30px">
            <div style="padding: 10px 20px; width: 200px; background-color: lightcyan;">
                <span style="font-size: 16px;">{{ __('ui.receipt_ref_no') }}</span>
            </div>
        ...
        <div style="margin-top:5px; display: block; background-color: rgba(92, 172, 237, 0.12); padding: 5px;">
            <span style="font-weight:bold; font-size: 20px; color: #000; padding-left: 10px">{{ __('ui.student_information') }}</span>
        </div>
       ...
       <div style="float: left; margin-left: 20px">
           <table style="font-size: 16px" class="td-left" cellspacing="5" cellpadding="5">
               <tr>
                   <td class="bold">{{ __('ui.name') }}:</td>
                   <td>{{ $sr->user->name }}</td>
               </tr>
               <tr>
                   <td class="bold">{{ __('ui.adm_no') }}:</td>
                   <td>{{ $sr->adm_no }}</td>
               </tr>
               <tr>
                   <td class="bold">{{ __('ui.class') }}:</td>
        ...
        {{-- Payment Info --}}
        <div style="margin-top:5px; display: block; background-color: rgba(92, 172, 237, 0.12); padding: 5px;">
            <span style="font-weight:bold; font-size: 20px; color: #000; padding-left: 10px">{{ __('ui.payment_information') }}</span>
        </div>

        <table class="td-left" style="font-size: 16px" cellspacing="2" cellpadding="2">
                <tr>
                    <td class="bold">{{ __('ui.reference') }}:</td>
                    <td>{{ $payment->ref_no }}</td>
                    <td class="bold">{{ __('ui.title') }}:</td>
                    <td>{{ $payment->title }}</td>
                </tr>
                <tr>
                    <td class="bold">{{ __('ui.amount') }}:</td>
                    <td>{{ $payment->amount }}</td>
                    <td class="bold">{{ __('ui.description') }}:</td>
        ...
        {{-- Payment Desc --}}
        <div style="margin-top:5px; display: block; background-color: rgba(92, 172, 237, 0.12); padding: 5px;">
            <span style="font-weight:bold; font-size: 20px; color: #000; padding-left: 10px">{{ __('ui.description') }}</span>
        </div>

        <table class="td-left" style="font-size: 16px" width="100%" cellspacing="2" cellpadding="2">
           <thead>
           <tr>
               <td class="bold">{{ __('ui.date') }}</td>
               <td class="bold">{{ __('ui.amount_paid') }} <del style="text-decoration-style: double">N</del></td>
               <td class="bold">{{ __('ui.balance') }} <del style="text-decoration-style: double">N</del></td>
           </tr>
           </thead>
            <tbody>
        ...
        <div class="bold arial" style="text-align: center; float:right; width: 200px; padding: 5px; margin-right:30px">
            <div style="padding: 10px 20px; width: 200px; background-color: lightcyan;">
                <span style="font-size: 16px;">{{ $pr->paid ? __('ui.payment_status') : __('ui.total_due') }}</span>
            </div>
            <div style="padding: 10px 20px; width: 200px; background-color: lightyellow;">
                <span style="font-size: 25px;">{{ $pr->paid ? __('ui.cleared') : $pr->balance }}</span>
            </div>
        </div>
        <div class="clear"></div>
    </div>
