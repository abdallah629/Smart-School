...
@section('page_title', __('ui.enter_pin'))
@section('content')
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title"><i class="icon-alarm mr-2"></i> {{ __('ui.enter_pin') }}</h5>
            {!! Qs::getPanelOptions() !!}
        </div>
                    ...
                    <form method="post" action="{{ route('pins.verify', Qs::hash($student->id)) }}">
                        ...
                        <div class="form-group">
                            <label for="pin_code" class="font-weight-bold col-form-label">
                                {{ __('ui.enter_exam_pin_for') }} <span class="text-success font-size-lg">{{ $student->name }}</span>
                            </label>
                        ...
                        <div class="text-center mt-2">
                            <button type="submit" class="btn btn-danger btn-lg">{{ __('ui.submit') }} <i class="icon-paperplane ml-2"></i></button>
                        </div>
